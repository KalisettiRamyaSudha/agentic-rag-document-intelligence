...
def load_document(
    ...
    *,
    ...
    path = _resolve_source_path(source_path, source_root)
    ...
    *,
    ...
    if source_path.is_dir():
        return _ingest_directory(
            source_path,
            source_root=source_root,
            version=_standalone_version(version),
...
def _ingest_manifest(
    ...
    *,
    ...
    resolved_version = version if version is not None else manifest.version
    ...
    for spec in manifest.documents:
        ...
        document = load_manifest_document(text, spec, version=resolved_version)
...
def _resolve_source_path(source_path: str | Path, source_root: str | Path | None) -> Path:
...
def _standalone_version(version: str | None) -> str:
    return "v1" if version is None else version
